...
void TIM7_IRQHandler(void)              //1ms 지나면 호출
{
  static int fnd_digit=1; //fnd 제일 앞에 있는 것.4ms마다 하나씩 다음걸 킴.

  if(TIM_GetITStatus(TIM7, TIM_IT_Update) != RESET)
  {
    TIM_ClearITPendingBit(TIM7, TIM_IT_Update);
    t_cnt++ ;
    systick_count++; //4ms마다 증가하는 전역변수. 즉 이 변수를 통해서 다른 일을 시킬 수 있음.
 
   // display_fnd(fnd_digit,systick_sec);  //1초에 1씩 증가하는 시간을 fnd에 출력시키겠다.
    display_fnd(fnd_digit,fndNumber);  
    if(fnd_digit == 4)          //4ms
      fnd_digit = 1;
    else
      fnd_digit++;  //fnd_dgit를 1 2 3 4 1 2 3 4로 반복시킴
