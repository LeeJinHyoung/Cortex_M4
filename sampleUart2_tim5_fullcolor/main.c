...
int main()
{
  ...
  TIM_TimeBaseStructure.TIM_Prescaler = 84-1;         //(168Mhz/2)/84 = 1MHz(1us) APB1이 84MHz이므로
  TIM_TimeBaseStructure.TIM_Period = 255-1;
  TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;
  TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
  TIM_TimeBaseInit(TIM5, &TIM_TimeBaseStructure);

  //반전모드를 한 이유:  common anod, 0일 때 불이 켜지는 회로를 구성하였기 대문이다.
  TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM2;     // 반전 모드 PWM의 값을 반전시킨다.
  TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
  TIM_OCInitStructure.TIM_Pulse = 0;        //초기 Duty 비를 100 으로 설정('0' 인부분 설정), 반전모드라 100%이다.
